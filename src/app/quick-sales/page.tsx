...
import { Plus, ShoppingCart, Trash2, Save, X, Calendar, User, Clock } from 'lucide-react'
...
interface AppointmentCheckoutData {
...
export default function QuickSalesPage() {
  ...
  const [appointmentData, setAppointmentData] = useState<AppointmentCheckoutData | null>(null)
  ...
  const checkAppointmentData = () => {
    const storedData = localStorage.getItem('appointmentCheckoutData')
    if (storedData) {
      try {
        const data: AppointmentCheckoutData = JSON.parse(storedData)
        if (data.fromAppointment) {
          setAppointmentData(data)
          setShowAppointmentInfo(true)
          
          // Pré-preencher o formulário com dados do agendamento
          setTimeout(() => {
            prefillFormFromAppointment(data)
          }, 1000) // Pequeno delay para garantir que os dados foram carregados
        }
      } catch (error) {
  ...
  const prefillFormFromAppointment = (data: AppointmentCheckoutData) => {
    // Criar item do serviço do agendamento
    const appointmentService: SaleItem = {
      id: `appointment-service-${Date.now()}`,
      type: 'service',
    ...
    setSaleForm(newSaleForm)
    setShowForm(true)
    
    toast.success('Dados do agendamento carregados!')
  }

  const clearAppointmentData = () => {
    localStorage.removeItem('appointmentCheckoutData')
  ...
  const addCustomItem = () => {
    const customName = prompt('Nome do item:')
    const customPrice = prompt('Preço:')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!saleForm.clientId || !saleForm.collaboratorId) {
      toast.error('Selecione cliente e colaborador')
      return
    }
    ...
    try {
      ...
      // - Atualizar estoque
      // - Atualizar status do agendamento (se vier de agendamento)
      
      if (appointmentData) {
        // Se veio de um agendamento, atualizar o status
        // await updateAppointmentStatus(appointmentData.appointmentId, 'CONCLUIDO')
        toast.success('Venda finalizada e agendamento concluído!')
  ...
  return (
    <ProtectedRoute requiredPermission={Permission.MANAGE_APPOINTMENTS}>
      ...
      <div className="p-6">
        ...
        {showAppointmentInfo && appointmentData && (
          <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-semibold text-blue-800 flex items-center gap-2">
                <Calendar className="h-5 w-5" />
                Checkout do Agendamento
              </h2>
              <button
                onClick={clearAppointmentData}
                className="text-blue-600 hover:text-blue-800"
                title="Limpar dados do agendamento"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-medium text-blue-800">{appointmentData.clientName}</p>
                  <p className="text-blue-600">{appointmentData.clientPhone}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-medium text-blue-800">{appointmentData.serviceName}</p>
                  <p className="text-blue-600">R$ {appointmentData.servicePrice.toFixed(2)}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-medium text-blue-800">{appointmentData.collaboratorName}</p>
                  <p className="text-blue-600">
                    {new Date(appointmentData.appointmentDateTime).toLocaleString('pt-BR')}
                  </p>
                </div>
              </div>
            </div>
            
            {appointmentData.appointmentNotes && (
              <div className="mt-3 p-2 bg-blue-100 rounded">
                <p className="text-sm text-blue-700">
                  <strong>Observações:</strong> {appointmentData.appointmentNotes}
                </p>
              </div>
            )}
            
            <div className="mt-3 text-xs text-blue-600">
              Os dados do agendamento foram pré-preenchidos. Você pode adicionar itens adicionais ou modificar conforme necessário.
            </div>
          </div>
        )}
                ...
                <div className="space-y-1 overflow-y-auto max-h-60">
                  {services.map(service => (
                    <button
                      ...
                      className="w-full p-2 hover:bg-gray-100 rounded-md text-sm bg-white border-pink-600 border justify-center items-center text-left"
                    >
                      <p className="font-medium">{service.name}</p>
                      <p className="text-pink-600">R$ {service.price.toFixed(2)}</p>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-6">
                ...
                <div className="space-y-1 max-h-40 overflow-y-auto">
                  {inventory.map(product => (
                    ...
                    >
                      ...
                      <p className="text-pink-600">R$ {product.unit_price.toFixed(2)}</p>
                    </button>
                  ))}
                </div>
                ...
                <div className="mt-3">
                  <button
                    ...
                    className="w-full p-2 border-2 border-dashed border-pink-300 rounded-lg text-pink-600 hover:border-pink-500 hover:text-pink-700 transition-colors"
                  >
